...
// deprecated-typed stub that only has { deprecated: true }. Guard everything with
// try/catch so a missing/renamed API in future Monaco versions cannot abort
// module initialization and crash the app before React renders.
try {
  ...
  if (tsLang && tsLang.typescriptDefaults && tsLang.javascriptDefaults) {
    ...
    console.warn(
      "[litecode] Monaco TypeScript language service is unavailable; skipping defaults."
    );
  }
} catch (err) {
  console.error("[litecode] Failed to configure Monaco TS/JS defaults:", err);
}
